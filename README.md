...
버전 **0.5.0**. 의뢰 작성은 가운데 정렬 주문서 카드입니다. 유형은 DB 카탈로그(시드 6종)입니다.
...
0.4.0에서 올라올 때 **마이그레이션이 필요합니다.** 유형 시드·새 컬럼·파일 테이블이 추가됩니다. 레이아웃 반영은 모듈 업데이트 후 **캐시를 비우고 브라우저를 하드 리프레시**하세요.
