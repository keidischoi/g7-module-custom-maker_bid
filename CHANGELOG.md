...
### Fixed
- **관리자 승인/보류 색**: G7가 클래스를 지워도 인라인 색으로 현재 상태를 칠합니다. 승인은 녹색, 보류는 노란색.
- **회사목록 필터**: G7 Select가 비어 보이던 칸을 네이티브 상태 셀렉트로 대체합니다.
- **의뢰목록 검색 백스페이스**: G7 Input을 화면 밖으로 밀어 네이티브 검색창만 쓰고, 입력 중 값을 덮지 않습니다.
- **정렬 빈 칸 + 검색 버튼**: 네이티브 검색/정렬/검색 버튼을 한 줄로 재배치합니다.
- **형식·상태 필터**: 1줄은 형식(유형) 칩, 2줄은 의뢰 상태 칩. 검색어가 남아 카드가 줄어들던 경우를 함께 고칩니다.
