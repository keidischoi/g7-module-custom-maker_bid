...
- **(임시)** `/maker-bid/new` 사이드 **임의입력** 버튼. 클릭 시 이미지·파일 업로드를 제외한 주문서 필드를 무작위 더미로 채웁니다(급행 켜면 조건 시각, 크기 1–3행, 담당자 정보 포함). **모듈 완성 후 삭제 예정.**
...
- 레이아웃·CSS 반영은 `php artisan module:update custom-maker_bid` 후 **캐시 삭제와 하드 리프레시**가 필요합니다. `rush_deadline` / `revision_*` / `sizes` / `manager_*` 컬럼이 없는 설치에는 additive 마이그레이션이 적용됩니다.
