...
- **관리자 의뢰 상세**: 승인·보류·분쟁조정·취소·삭제·의뢰 저장이 목록과 같이 Bearer로 동작합니다. 상태·공개 설정은 네이티브 select라 Radix가 클릭을 잠그지 않습니다. 분쟁조정 `data-cmb-kind`가 상태 칠을 깨뜨려 버튼 바인딩이 멈추던 문제도 고칩니다.
- **선택 업체 관리**: 불러온 업체의 스칼라 값만 스냅샷에 둡니다. 로고 파일 배열을 setState에 다시 넣지 않아 업로더 Div 오류가 나지 않고, 제목 `#번호`는 G7 밖에서 그립니다.

### Changed
- 관리자 업로더 박스를 한 줄 안내로 접습니다. 큰 업로드 아이콘·부가 문구는 숨깁니다.
- 버전·캐시 버스트 **0.10.38**.

## [0.10.37] - 2026-09-18

### Fixed
- **관리자 회사목록**: 선택 업체 관리 폼에 빠진 프로필 항목(이름·유형·사업자번호·소개·URL·담당자·연락처·주소·주력유형)을 왼쪽에 모두 넣고, 불러오기·저장이 수정값을 반영합니다.

### Changed
- 로고 이미지는 의뢰주문서처럼 업로드 박스 안에 보입니다.
- 승인·보류·거절을 **선택 업체 저장**과 같은 줄에 둡니다.
- 버전·캐시 버스트 **0.10.37**.
...
### Fixed
- **관리자 의뢰 목록 버튼**: G7 상태 Select가 Radix 포털을 남기며 페이지 클릭을 잠그던 문제를 고칩니다. 유형·상태는 네이티브 `<select>`로 바꾸고, 상세/수정·승인·보류·취소·삭제·필터가 다시 동작합니다.
- 다크 관리자에서 드롭다운 글자가 안 보이던 문제를 고칩니다(어두운 배경·밝은 글자).
...
- **업체 결제정보**: 입찰자 등록에 은행·계좌·예금주와 기본 계약금 비율·조건을 넣습니다. 계좌는 본인·관리자에게만 보입니다.
- **낙찰 계약금**: 낙찰 시 계약금 비율(요청 > 업체 기본 > 설정 기본 30%)로 계약금·잔금이 나뉩니다. 잔금은 계약금 확인 뒤에만 신고할 수 있고, 완료는 모든 입금 확인 뒤입니다.
- **의뢰자 환불 계좌**: 의뢰서 개인정보에 선택으로 환불 은행·계좌 소유자명·계좌번호를 넣습니다. 낙찰된 제작자와 관리자에게만 보입니다.
- **사이트 알림**: 입찰·낙찰·결제·분쟁 이벤트를 G7 기본 알림(홈페이지 벨, `notifications` 테이블)에 넣습니다. 이메일은 기존처럼 가고, 모듈 `/maker-bids/notices` 기록도 유지합니다.
- 관리자 설정에서 **이메일 알림** / **사이트 알림**을 따로 켤 수 있습니다.
...
### Fixed
- **의뢰 취소**: 관리자 상태 저장이 `cancelled`를 견적요청으로 되돌리던 문제를 고칩니다. 취소 버튼과 상태 선택 모두 취소로 남고, 입찰은 종료됩니다.
- 의뢰목록 한글 필터 `취소`/`완료`/`낙찰`이 빈 목록이 되지 않습니다.

### Changed
- 취소된 의뢰의 입찰 라벨은 **종료**입니다.
- 버전·캐시 버스트 **0.10.32**.
